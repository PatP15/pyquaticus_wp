...
if __name__ == '__main__':
    ...
    reward_config = {0:rew.west_point_defend0, 1:None} # Example Reward Config
    ...
    def policy_mapping_fn(agent_id, episode, worker, **kwargs):
        if agent_id == 1:
            selection = np.random.choice(["rand-policy", "easy-attack", "medium-attack", "hard-attack"], p=probabilities)
            # print(selection)
            return selection #np.random.choice(["attacker-0-policy", "rand-policy"])
        else:
            return "defender-0-policy"
    ...
    'easy-attack':(bp.AttackGen(1, Team.RED_TEAM, 'easy', 1), obs_space, act_space, {}),
    'medium-attack':(bp.AttackGen(1, Team.RED_TEAM, 'medium', 1), obs_space, act_space, {}),
    'hard-attack':(bp.AttackGen(1, Team.RED_TEAM, 'hard', 1), obs_space, act_space, {})
    }
    # else:
    #     policies = {
    #     'attacker-1-policy':(None, obs_space, act_space, {}), 
    ...
    #     }
    ppo_config = PPOConfig().environment(env='pyquaticus').rollouts(num_rollout_workers=4).resources(num_cpus_per_worker=1, num_gpus=int(os.environ.get("RLLIB_NUM_GPUS", "0")))
    ...
    ppo_config.multi_agent(policies=policies, policy_mapping_fn=policy_mapping_fn, policies_to_train=["defender-0-policy"],)
    algo = ppo_config.build()

    if args.checkpoint != "":
        algo.restore('./ray_single_defend/'+str(args.checkpoint)+'/')
        # algo = Algorithm.from_checkpoint(
        #     checkpoint='./ray_single_defend/'+str(args.checkpoint),
        #     policy_ids={"defender-0-policy", "rand-policy", "easy-attack", "medium-attack", "hard-attack"},  # <- restore only those policy IDs here.
    ...
    for i in range(10000):
        algo.train()
        print("Iter: " + str(i))
        if np.mod(i, 50) == 0 and i > 10:
            chkpt_file = algo.save('./ray_single_defend/')
